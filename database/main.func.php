...
// Login validation
function adminLogin($con, $email, $pwd, $table)
{
    ...
    if ($checkPwd === false) {
        ...
        if ($_SESSION["role"] == "Police Officer") {
            header("location: ../../public/police_officer/po_dashboard.php");
            exit();
        }
        if ($_SESSION["role"] == "Police Station Admin") {
            header("location: ../../public/police_station/dashboard.php");
            exit();
        }
        if ($_SESSION["role"] == "RMV Admin") {
            header("location: ../../public/rmv_admin/rmv_dashboard.php");
            exit();
        } else {
            header("location: ../../index.php?error=invalidLogin");
            exit();
...
function emailExists($con, $email)
{
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: ../../public/system-admin/sa-register.php?error=stmtFailed");
        exit();
    }
...
function adminRegister($con, $email, $pwd, $role, $table)
{
