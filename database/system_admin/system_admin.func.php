...
function lawAdd($con, $act, $part_number, $chapter_number, $section_number, $title, $law_text, $law_type, $fine_amount, $points_deducted, $user_id)
{
    ...
    $stmt = mysqli_stmt_init($con);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: /efine-merged/public/system_admin/law-add.php?error=stmtFailed");
        exit();
    }
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("location: /efine-merged/public/system_admin/law-view.php?error=none");
    exit();
}
...
function lawGetData($con)
{
    if (!isset($_GET['id']) || !ctype_digit($_GET['id'])) {
        header("location: /efine-merged/public/system_admin/law-view.php?error=stmtFailed");
        exit();
    }
...
function lawUpdate($con, $id, $fine_amount, $points_deducted, $user_id)
{
    ...
    if ($update_query) {
        header("location: /efine-merged/public/system_admin/law-view.php?error=updated");
        exit();
    } else {
        header("location: /efine-merged/public/system_admin/law-view.php?error=cantUpdate");
        exit();
    }
}
...
{
    if (!isset($_GET['id']) || !ctype_digit($_GET['id'])) {
        header("location: /efine-merged/public/system_admin/law-view.php?error=cantDelete");
        exit();
    }
    ...
    if (mysqli_stmt_affected_rows($stmt) > 0) {
        header("location: /efine-merged/public/system_admin/law-view.php?error=deleted");
        exit();
    } else {
        header("location: /efine-merged/public/system_admin/law-view.php?error=cantDelete");
        exit();
...
function psaAdd($con, $province, $district, $police_station, $email, $court_name, $user_id)
{
    ...
    $stmt = mysqli_stmt_init($con);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: /efine-merged/public/system_admin/psa-add.php?error=stmtFailed");
        exit();
    }
...
{
    if (!isset($_GET['id']) || !ctype_digit($_GET['id'])) {
        header("location: /efine-merged/public/system_admin/psa-view.php?error=stmtFailed");
        exit();
    }
...
function psaUpdate($con, $email)
{
    ...
    if ($update_query) {
        header("location: /efine-merged/public/system_admin/psa-view.php?error=updated");
        exit();
    } else {
        header("location: /efine-merged/public/system_admin/psa-view.php?error=cantUpdate");
        exit();
    }
}
...
{
    if (!isset($_GET['id']) || !ctype_digit($_GET['id'])) {
        header("location: /efine-merged/public/system_admin/psa-view.php?error=cantDelete");
        exit();
    }
    ...
    if (mysqli_stmt_affected_rows($stmt) > 0) {
        header("location: /efine-merged/public/system_admin/psa-view.php?error=deleted");
        exit();
    } else {
        header("location: /efine-merged/public/system_admin/psa-view.php?error=cantDelete");
        exit();
...
function rmvAdd($con, $name, $email, $user_id)
{
    ...
    $stmt = mysqli_stmt_init($con);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: /efine-merged/public/system_admin/rmv-add.php?error=stmtFailed");
        exit();
    }
...
function rmvDelete($con)
{
    if (!isset($_GET['id']) || !ctype_digit($_GET['id'])) {
        header("location: /efine-merged/public/system_admin/rmv-view.php?error=cantDelete");
        exit();
    }

    $id = $_GET['id'];
    $sql = "DELETE FROM rmv_admin WHERE id = ?;";
    $stmt = mysqli_stmt_init($con);
    ...
    if (mysqli_stmt_affected_rows($stmt) > 0) {
        header("location: /efine-merged/public/system_admin/rmv-view.php?error=deleted");
        exit();
    } else {
        header("location: /efine-merged/public/system_admin/rmv-view.php?error=cantDelete");
        exit();
